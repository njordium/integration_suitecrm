...
Rationale — the widget surface is mature, well-tested, and has one clear job: read from SuiteCRM and render nine dashboards. The write / in-context surface is coupled to Mail / Talk / Files / Deck app versions, has its own bug surface (see the 3.2.0-iteration `FileAction is not a constructor` regression, the follow-up-Task widget re-wire that broke on Vue rewrite, the Nextcloud Mail extension-API-absent block), and belongs in its own release cycle. Splitting them lets users install only what they need and lets each app move at its own pace.

### Removed (moved to `integration_suitecrm_bridge`)

- Quick Actions floating action button + `Cmd/Ctrl+Shift+K` keyboard shortcut (`QuickActionsFab.vue`, `quickActions.js`, `AddQuickActionsScriptListener.php`).
- Talk-to-Note modal + Log-Talk-conversation flow (`TalkToNoteModal.vue`, `talkHook.js`, `LoadTalkScriptListener.php`).
- Deck-to-SuiteCRM link flow (`LinkDeckCardModal.vue`).
- Email-to-Case flow (`EmailToCaseModal.vue`).
- Follow-up-Task modal + widget-row hover-reveal button (`TaskFollowupModal.vue`, per-row `NcButton` on Calendar / Events widgets, `.scw-item__followup` CSS).
- Files action "Link to SuiteCRM record" + Talk-artefact smart variant (`filesHook.js`, `FileToSuiteCRMDialog.vue`, `LoadFilesScriptListener.php`).
- SuiteCRM record picker (`SuiteCRMRecordPicker.vue` — only used by the moved modals).
- Personal-settings "Quick actions to SuiteCRM" panel + related icons + the `quick_actions_enabled` toggle handler.
- Write endpoints on `SuiteCRMAPIController`: `createFollowupTask`, `logNote`, `linkDeckCard`, `emailToCase` + their `UserRateLimit` attribute import.
- All test coverage tied to the four write endpoints — 51 tests / ~424 assertions moved to the bridge app together with the source.

### What stays (this app's job going forward)

- Nine dashboard widgets + `SuiteCRMWidgetShell` + per-widget settings menu (refresh cadence, records-to-show, `Only records assigned to me`).
- SuiteCRM connection: OAuth authorization-code flow, `TokenStorage`, admin settings panel, personal-settings `Connected accounts` panel + `override_user_name` field.
- Reminder notifications background job.
- Unified search provider.
- Reference provider (paste SuiteCRM URL → preview card).
- Companion Calendar-sync module setup panel.
- `occ integration_suitecrm:test-connection` diagnostic.
- OWASP-hardened widget backend endpoints unchanged from 3.1.0 (rate-limit clamps, `clampLimit()`, `sniffImageMime()` on avatar, whitelisted `USER_ALLOWED_KEYS`).

### Notes for maintainers

- Semver — 3.2.0 removes user-visible surfaces (the FAB, all four write flows) which is technically breaking. The tag is force-moved from the 3.1.x-iteration content to this pivot; because 3.2.0 was never App Store submitted, this rewrite is safe. Anyone who cloned the previous 3.2.0 head gets the split on the next `git fetch --tags --force`.
- Bridge app namespace uses `suitecrm_bridge_*` for admin/user config keys to prevent collisions with this app's `integration_suitecrm.*` prefs. The two apps' OAuth flows are independent per architectural decision — user re-authorises in the bridge app.
- The scratchpad staging directory `scratchpad/integration_suitecrm_bridge/` in the dev environment carries the extracted source, ready to be committed into the new repo when it's set up. See `README-STAGING.md` there for the migration plan.
- No config-schema changes on the widget side; existing installs pick up 3.2.0 without a migration step. The removed keys (`quick_actions_enabled`, four write-flow keys) linger in `oc_appconfig` / `oc_preferences` harmlessly and can be pruned by a follow-up repair step if needed.
...
- **Vue settings anchors** — `src/components/PersonalSettings.vue` and `src/components/AdminSettings.vue` inline the same collapsed rule so the little icon next to the "Connected accounts" / admin settings heading matches the dashboard widgets.
...
Settings modal exposes three (four for Calendar / Pipeline) sections per widget:
- **Refresh frequency** — Never / 30s / 1m / 5m / 15m / 30m / 1h (default 5 minutes)
- **Records to show** — 5 / 10 / 15 / 20 / 25 / 50 (default 20). Doubles as the fetch `limit` sent to the SuiteCRM API so we don't ask for rows we'd drop client-side; server clamps to `max(1, min(100, N))` as defence against a tampered client.
- **Show → Only records assigned to me** — checkbox on eight of nine widgets. Defaults ON for Calendar / Cases / Tasks / Pipeline (matches the existing single-user behavior; toggle OFF widens to team-visible records within your SuiteCRM ACL). Defaults OFF for Activities / Contacts / Accounts / Leads (they've always shown any recently-added record; toggle ON filters to your own). Events (reminders) is per-user by SuiteCRM design and has no toggle.
- **Widget-specific extras** — Calendar carries "Include Tasks alongside Meetings and Calls"; Pipeline carries the "Closing this quarter / Top value / Weighted value" framing radio (moved out of PersonalSettings.vue's global block).
...
- **Per-widget settings moved out of `PersonalSettings.vue` and into the widget's own 3-dot menu** where they belong:
  ...
  - Pipeline widget: `pipeline_mode` radio selector (closing this quarter / top value / weighted)
  The keys themselves are unchanged, so a 3.0.x install upgrading to 3.1.0 keeps whatever value the user had picked.
