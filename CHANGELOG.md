...
**In-context Quick Actions.** Kills the copy-paste UX for logging Nextcloud content into SuiteCRM. Instead of opening a modal from the floating action button and pasting a URL, users invoke SuiteCRM actions directly from the native context of the app they're already in: right-click a file in Files, click a message's kebab in Talk, or use the hover-reveal `+` on a Calendar / Events widget row. All flows use the same authenticated `/log-note` and `/task-followup` backend endpoints as 3.1.0 — only the launch surface is new.

### Added

- **Files action "Link to SuiteCRM record"** — registers a per-file action via `@nextcloud/files` `registerFileAction()` (Nextcloud 28+ new file-action API). Click → picker dialog for the SuiteCRM record + optional note title + extras → POST creates a Note on the picked record with the file's canonical `/f/{fileid}` link as the body. Link-only mode: no file bytes cross the boundary, Nextcloud stays the single source of truth. New files: [`src/filesHook.js`](src/filesHook.js), [`src/components/FileToSuiteCRMDialog.vue`](src/components/FileToSuiteCRMDialog.vue), [`lib/Listener/LoadFilesScriptListener.php`](lib/Listener/LoadFilesScriptListener.php).
- **Talk-artefact smart variant** on the same Files action — files under `/Talk/Recording/…` or carrying spreed's "Generated by AI" system tag get the action label swapped to *"Log this Talk artefact as a SuiteCRM Meeting Note"* and a pre-filled note title that reflects the source. Detection is defensive: a Files install without the systemtags integration silently keeps the generic label. Rationale documented — current spreed does not write per-conversation `.md` transcripts to Files (call recordings are the only file artefact, AI summaries land as chat messages), so the smart-variant covers today's reality without depending on a feature that doesn't ship yet.
- **Talk in-context action "Log conversation to SuiteCRM"** — registers a per-message action via `window.OCA.Talk.registerMessageAction` (Talk 15+ / Nextcloud 22+ public API). Kebab menu on any Talk message → the shared `TalkToNoteModal` opens pre-filled with the conversation token + name, so the user only picks the SuiteCRM record + how many recent messages to include. Message fetch continues to go through the existing spreed OCS `/chat/{token}` endpoint. Load flow: `BeforeTemplateRenderedEvent` filtered on the `/call/` pathinfo (spreed dispatches no first-party `LoadAdditionalScripts` event, so this is the canonical third-party pattern used by nextcloud/bookmarks' Talk hook and spreed's own DeckPluginLoader). New files: [`src/talkHook.js`](src/talkHook.js), [`lib/Listener/LoadTalkScriptListener.php`](lib/Listener/LoadTalkScriptListener.php).
- **Follow-up-Task action re-wired on dashboard widget rows** — regression fix from the 3.1.0 Vue widget rewrite where `TaskFollowupModal.vue` was orphaned and the docs still advertised the feature (README line 55). Every row on the Calendar and Events widgets now carries a hover-reveal `+` button; click opens the modal pre-filled with the source row's SuiteCRM module + record id + label. The created Task auto-links back on the SuiteCRM side via `parent_type` / `parent_id`. Shared `.scw-item__followup` CSS in [`css/dashboard.css`](css/dashboard.css) so any widget can adopt the same affordance by dropping the class into a row template.

### Deferred / documented as out of scope

- **Mail in-context "Send to SuiteCRM as Case" action** — investigated and **blocked upstream**. Nextcloud Mail (`nextcloud/mail`, checked at `main` — `5.12.0-alpha.1`) exposes no third-party JS registration point for message actions. Every button in the message kebab (`MenuEnvelope.vue`) is a hardcoded `<NcActionButton>` compiled into the Vue tree. Confirmed by grepping the Mail source: no `OCA.Mail.registerMessageAction`, no action bus, no plugin loader, and no `LoadAdditionalScripts` event equivalent. Reference integrations (`integration_openproject`, `integration_github`, `integration_openstreetmap`) do not hook Mail at all. Fallbacks — reference-provider / smart-picker widget in the Mail composer, or waiting for an upstream extension API — are documented in the code review; a monkey-patched DOM injection was considered and rejected as too fragile across Mail versions. The existing FAB modal remains the "convert email to Case" path until upstream ships an extension API.

### Notes for maintainers

- The Mail non-integration is a known limitation that will remain until Nextcloud Mail publishes a public message-action registration API. If a user asks for the feature, the ask is well-formed: point them at upstream and offer the FAB modal as the interim.
- `LoadFilesScriptListener` uses a string class-name check on the `OCA\Files\Event\LoadAdditionalScriptsEvent` FQCN rather than a `use`d import so `nextcloud/ocp`'s phpstan stubs (which don't ship the Files-app class) build clean without needing a compose-time dependency on the Files app.
- `LoadTalkScriptListener` filters on the request path prefix `/call/` — matches spreed's own route pattern. If spreed ever changes its base route the filter has to move; the canonical location to check is spreed's `appinfo/routes.php`.
- When adding a new in-context action to a fourth Nextcloud app: mirror this pattern — `src/<app>Hook.js` for the JS registration, `lib/Listener/Load<App>ScriptListener.php` for the load-scoping event listener, one entry in `webpack.js`, one line in `Application::register()`. The pattern is deliberately uniform to keep the mental model portable.
