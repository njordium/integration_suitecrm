...
npm ci
npm run build
sudo -u www-data php /var/www/nextcloud/occ app:enable integration_suitecrm
```

**Release-zip install:** grab `integration_suitecrm-<version>.zip` from the [Releases page](https://github.com/njordium/integration_suitecrm/releases), extract into `custom_apps/`, enable the app. Zip already includes the built `js/` bundles, no `npm run build` needed.

Verify the app is enabled: **Settings → Administration → Overview** shows no complaints about `integration_suitecrm`, and `Settings → Administration → Connected accounts` has a **SuiteCRM integration** section.
...
6. **Redirect URL:** exactly `<your-nextcloud-public-url>/apps/integration_suitecrm/oauth-callback`. **Byte-for-byte match matters** — `http` vs `https`, port number, trailing slash, all count. If your Nextcloud is `https://cloud.example.com`, this is `https://cloud.example.com/apps/integration_suitecrm/oauth-callback`.
...
If you hit something not covered there, `occ integration_suitecrm:test-connection` from step 5 is the fastest first-line diagnostic. Its output tells you which layer is broken.
