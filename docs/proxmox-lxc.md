...
Then in SuiteCRM's admin UI, create the OAuth2 Client (Admin → OAuth2 Clients and Tokens → New), set redirect URL to `https://cloud.example.com/apps/njordium_suitecrm/oauth-callback`, note the client ID + secret, and configure them in Nextcloud's admin panel with instance URL `https://crm.example.com`.
...
```bash
tail -f /var/www/nextcloud/data/nextcloud.log | grep njordium_suitecrm
```
