...
3. The whitelisted one is `https://cloud.example.com/apps/njordium_suitecrm/oauth-callback` (as it must be for the browser to reach it)
...
$url = \OC::$server->getURLGenerator()->linkToRouteAbsolute(
    "njordium_suitecrm.config.oauthCallback"
);
echo $url . "\n";
'
# Expected output: https://cloud.example.com/apps/njordium_suitecrm/oauth-callback
